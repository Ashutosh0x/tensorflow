...
CompileCpuExecutableInternal(
    ...
    const AotCompilationOptions* absl_nullable aot_options, int process_index,
    bool collectives_exists,
    std::function<void(HloModuleConfig&)> customize_hlo_module_config) {
  ...
  if (!collectives_exists && device_assignment) {
  ...
  if (aot_options) {
    ...
    if (!thread_pool) {
      thread_pool = compile_thread_pool;
    }
...
absl::StatusOr<std::unique_ptr<PjRtCpuExecutable>> CompileCpuExecutable(
  ...
  ASSIGN_OR_RETURN(
      ...
      CompileCpuExecutableInternal(
          ...
          topology, /*num_threads=*/std::nullopt,
          /*compile_thread_pool=*/nullptr, /*aot_options=*/nullptr,
          /*process_index=*/0, /*collectives_exists=*/true,
...
absl::StatusOr<std::unique_ptr<PjRtCpuExecutable>> CompileCpuExecutable(
  ...
  ASSIGN_OR_RETURN(
      ...
      CompileCpuExecutableInternal(
          ...
          /*num_threads=*/std::nullopt, /*compile_thread_pool=*/nullptr,
          /*aot_options=*/nullptr, /*process_index=*/0,
          /*collectives_exists=*/true, std::move(customize_hlo_module_config)));
  return std::move(results.first);
}
...
PjRtCpuClient::CompileInternal(
  ...
  return CompileCpuExecutableInternal(
      computation, argument_layout_pointers, layout_canonicalization_callback,
      std::move(options), *topology_, num_threads, eigen_intraop_pool(),
      aot_options, process_index(), collectives_ != nullptr,
