...
absl::StatusOr<std::unique_ptr<PjRtExecutable>> CpuPjRtCompiler::Compile(
  ...
  if (client != nullptr && client->platform_id() == xla::CpuPlatformId()) {
    return client->Compile(computation, std::move(options));
  }
...
absl::StatusOr<std::unique_ptr<PjRtExecutable>> CpuPjRtCompiler::Compile(
  ...
  if (client != nullptr && client->platform_id() == xla::CpuPlatformId()) {
    return client->Compile(std::move(module), std::move(options));
  }
